...
%% @private
-spec get_conn_info() ->
    {ok, nkhttpc:conn_opts()|#{ip=>inet:ip_address()}} | {error, invalid_host}.
...
%% @private
-spec get_conn_info(nkhttpc:conn_opts()) ->
    {ok, nkhttpc:conn_opts()|#{ip=>inet:ip_address()}} | {error, invalid_host}.

get_conn_info(Opts) ->
    {ok, EnvConfig} = application:get_env(nkhttpc, conn_config),
...
remove_exited() ->
	Op = fun(Pid) ->
		case nkhttpc:ps(Pid, #{filters=>#{status=>[exited]}}) of
			{ok, List} ->
				Ids = [Id || #{<<"Id">>:=Id} <- List],
...
build(Tag, TarBin) ->
	Op = fun(Pid) ->
		case nkhttpc:inspect_image(Pid, Tag) of
	    	{ok, _} ->
	    		ok;
			{error, {not_found, _}} ->
				lager:notice("Building docker image ~s", [Tag]),
				Timeout = 3600 * 1000,
	    		case nkhttpc:build(Pid, TarBin, #{t=>Tag, async=>true, timeout=>Timeout}) of
	    			{async, Ref} ->
...
make_tar(List) ->
	list_to_binary([nkhttpc_tar:add(Path, Bin) || {Path, Bin} <- List]).
...
wait_async_iter(Ref, Mon, Timeout) ->
	receive
		{nkhttpc, Ref, {data, #{<<"stream">> := Text}}} ->
			io:format("~s", [Text]),
			wait_async_iter(Ref, Mon, Timeout);
		{nkhttpc, Ref, {data, #{<<"status">> := Text}}} ->
			io:format("~s", [Text]),
			wait_async_iter(Ref, Mon, Timeout);
		{nkhttpc, Ref, {data, Data}} ->
			io:format("~p\n", [Data]),
			wait_async_iter(Ref, Mon, Timeout);
		{nkhttpc, Ref, {ok, _}} ->
			ok;
		{nkhttpc, Ref, {error, Reason}} ->
			{error, Reason};
		{nkhttpc, Ref, Other} ->
