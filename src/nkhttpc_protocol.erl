...
conn_init(NkPort) ->
	{ok, {notify, Pid}} = nkpacket:get_user_state(NkPort),
...
conn_encode({http, Ref, Method, Path, Hds, Body}, _NkPort, State) ->
	request(Ref, Method, Path, Hds, Body, State);
...
request(Ref, Method, Path, Hds, Body, #state{streams=Streams}=State) ->
	...
	State2 = State#state{streams = Streams ++ [Ref]},
	RawMsg = cow_http:request(Method, Path, 'HTTP/1.1', Hds3),
...
handle(Data, #state{next=chunked}=State) ->
	...
	case parse_chunked(Data1) of
		{data, <<>>, Rest} ->
			Pid ! {nkhttpc, Ref, {body, <<>>}},
			handle(Rest, do_next(State));
		{data, Chunk, Rest} ->
			Pid ! {nkhttpc, Ref, {chunk, Chunk}},
...
handle_head(Data, #state{streams=[Ref|_], notify=Pid}=State) ->
	...
	Pid ! {nkhttpc, Ref, {head, Status, Hds}},
